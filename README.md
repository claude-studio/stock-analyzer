...
- **종목 관계 온톨로지**: Claude 기반 관계 시드 생성(경쟁/공급/고객/계열/동종) + 뉴스 기반 자동 갱신
- **영향 체인 추론**: 관계사 동향이 분석 종목에 미치는 간접 영향 CoT 추론
- **2차 영향 분석**: 감성분석 시 관계사 수혜/피해 종목 자동 추출
...
| `GET /api/v1/stocks/{ticker}/relations` | 종목 관계 목록 | X-API-Key |
| `POST /api/v1/relations/seed` | 관계 시드 생성 트리거 | X-API-Key + Rate Limit |
