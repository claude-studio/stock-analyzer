...
형식:
[
  {"index": 0, "sentiment": "positive", "score": 0.85, "tickers": ["005930"]},
  {"index": 1, "sentiment": "negative", "score": 0.72, "names": ["카카오"]},
  ...
]
...
- secondary_impacts: 관계사에 미치는 2차 영향. 없으면 빈 배열.
  형식: [{{"name": "SK하이닉스", "relation": "competitor", "direction": "bullish"}}]
...
async def analyze_sentiment_batch(
    ...
    Args:
        ...
        batch_size: 배치 크기
        relation_context: 종목 관계 컨텍스트 (2차 영향 분석용)
    ...
    for batch_start in range(0, len(headlines), batch_size):
        ...
        try:
            ...
            for item in parsed:
                ...
                item.setdefault("secondary_impacts", [])
...
async def update_news_sentiment(
    ...
    for mapping in mappings:
        ...
        if matcher:
            ...
            # secondary_impacts 저장
            secondary = s_item.get("secondary_impacts", [])
            if secondary and isinstance(secondary, list):
                from app.service.db_service import save_news_impact as _save_impact
                for sec in secondary:
                    sec_name = sec.get("name", "")
                    sec_matched = matcher.match(sec_name) if sec_name else None
                    if not sec_matched:
                        continue
                    sec_direction = sec.get("direction", "neutral")
                    if sec_direction not in ("bullish", "bearish", "neutral"):
                        sec_direction = "neutral"
                    sec_reason = sec.get("reason", "")
                    rel_type = sec.get("relation", "")
                    reason_text = f"[{rel_type}] {sec_reason}" if rel_type else sec_reason
                    await _save_impact(
                        session,
                        news_article_id=mapping["article_id"],
                        stock_id=sec_matched[0],
                        impact_direction=sec_direction,
                        impact_score=None,
                        reason=reason_text,
                    )
