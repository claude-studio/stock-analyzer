    ...
    <div className="min-w-0 space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-400">
          사용자에게 노출되는 최종 일일 추천만 집계하며, analyst shard와 on-demand 리포트는 제외합니다.
        </p>
      </div>
      ...
      <div className="rounded-lg border border-yellow-500/20 bg-yellow-500/5 px-4 py-3">
        <p className="text-sm text-yellow-400">
          적중률은 과거 최종 일일 추천 결과의 통계이며, 미래 수익을 보장하지 않습니다. 투자 판단은 본인 책임입니다.
        </p>
      </div>
      ...
      {stats ? (
        <>
          {hasEvaluatedStats ? (
            <>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                ...
                <StatCard
                  ...
                  sub="추천일 기준 7일 후 방향 일치"
                />
                <StatCard
                  label="30일 적중률"
                  value={`${(stats.hit_rate_30d * 100).toFixed(1)}%`}
                  sub="추천일 기준 30일 후 방향 일치"
                />
              </div>

              {/* 추천별 적중률 */}
              <div>
                <h2 className="mb-4 text-lg font-semibold">추천 유형별 적중률</h2>
            ...
            <div className="rounded-lg border border-gray-800 bg-[#111111] p-6">
              <p className="text-sm text-gray-300">선택한 기간에는 평가가 끝난 최종 일일 추천이 아직 없습니다.</p>
              <p className="mt-1 text-xs text-gray-500">
                analyst shard, on-demand 리포트, 미평가 추천은 이 통계에 포함되지 않습니다.
              </p>
            </div>
          )}
        </>
        ...
        <div className="rounded-lg border border-gray-800 bg-[#111111] p-6">
          <p className="text-sm text-gray-400">최종 일일 추천 적중률 데이터를 불러올 수 없습니다.</p>
          <p className="mt-1 text-xs text-gray-500">API 서버가 실행 중이고 평가된 daily 추천 이력이 있는지 확인하세요.</p>
        </div>
      )}
    </div>
  );
